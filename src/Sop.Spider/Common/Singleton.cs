...
namespace Sop.Spider.Common
{
    ...
    public abstract class Singleton<T>
    {
        private static readonly Lazy<T> MyInstance = new Lazy<T>(() =>
        {
            ...
            if (ctors.Length != 1)
            {
                throw new SpiderInvalidOperationException($"Type {typeof(T)} must have exactly one constructor.");
            }
            var ctor = ctors.SingleOrDefault(c => !c.GetParameters().Any() && c.IsPrivate);
            if (ctor == null)
            {
                throw new SpiderInvalidOperationException($"The constructor for {typeof(T)} must be private and take no parameters.");
            }
